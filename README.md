...
plugins). Skills, hooks, agents, rules and the CLAUDE.md template all install by a git-clone-and-copy
step built into the installers - one shallow clone per run, so an install is a single source
revision (recorded in `.claude/claude-stack.stamp`). The **Cursor**
...
| **git** | project-scope path resolution (repo root); cloning the stack source - skills, hooks, agents, rules and the CLAUDE.md template are all copied out of that one clone | **Yes** | `brew install git` | `winget install Git.Git` |
...
| -------- | ------ | ------- |
| **action** (positional) | `install` \| `update` (required) | `install` adds everything (idempotent - existing items skipped, MCP versions frozen). `update` brings everything to latest (clean re-add of skills, plugin/MCP refresh, re-copy of hook/agent/rule files). Both take ONE shallow clone of the stack source and copy every file out of it, then record that commit in `.claude/claude-stack.stamp`. |
...
| **`--source <dir>`** \| **`-Source <dir>`** | path | Install **from an existing claude-stack checkout** instead of cloning one. The caller owns `<dir>` - the script never deletes it. This is how the `/claude-stack` plugin skills work: they must clone anyway (for `stack-select.js`, the graph, the CLAUDE.md template and the stamp diff), so they pass that same clone here and a guided run costs **one** clone, not two. Omit it - the standalone path - and the script clones its own source and removes it on exit. |
