...
description: "Personal Angular conventions from v17 up - standalone everything, signals as the default state primitive, OnPush and zoneless, block control flow (at-if / at-for / at-switch), signal inputs and outputs, deferred loading, RxJS only where streams earn it, HTTP, reactive or signal forms, routing with input binding, SSR and hydration, accessibility, component-harness testing, and banned patterns. Load before writing or editing any Angular file so the agent commits to current idioms, not recalled ones. Companions: typescript for the language baseline (strict typing, modules, async, errors), angular-material for Material and CDK, angular-styling for encapsulation, tokens, and a11y styling, angular-security for XSS, sanitization, and auth, frontend for the web index, mobile for Ionic and Capacitor. Do NOT load for React, Vue, Svelte, Solid, plain DOM, or any non-Angular TypeScript."
---
...
- **Hand-rolled `resource` / `rxResource` / `httpResource`** for declarative reads. `httpResource` (introduced v19.2, stable from v22) is the lean default - `httpResource(() => \`/api/order/${id()}\`)` re-fetches when the signal moves, cancels the in-flight request, and exposes `value()`, `hasValue()`, `isLoading()`, and `error()` as signals, killing the manual loading/error booleans. Use `resource` for non-HTTP async and `rxResource` when the loader is an Observable (note: on v20 its loader was renamed `stream`, and rxResources stream values from v19.2 - version-tag any snippet). These all shipped experimental (resource / rxResource in v19, httpResource in v19.2) and reached stable in v22; pin to the installed version before leaning on them.
...
- Enable full hydration with `provideClientHydration()`. Incremental hydration is stable from v20, but the wiring changed at v22: through v21 you opt in with `withIncrementalHydration()`; from v22 it is on by default, that helper is deprecated (removal targeted v24), and you opt *out* with `withNoIncrementalHydration()`. Either way it auto-enables event replay, so do not also add `withEventReplay()`. Drive it from `@defer` with `hydrate` triggers - `hydrate on idle|immediate|timer|viewport|interaction|hover`, `hydrate when`, `hydrate never` - which lets you defer even above-the-fold content a plain `@defer` could not. (See your version's delta file under `references/` for the exact wiring.)
