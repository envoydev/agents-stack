...
- **.NET 10+:** first-party validation can run from `[Required]` and friends without a third-party library, but it is attribute-per-property only - no cross-field rule (this date after that one) and no async rule (uniqueness check against the database) without writing a custom `ValidationAttribute`, and an async one is not supported at all on that path. For anything beyond a single-field DTO the validator-in-a-filter approach above still carries those cases and reads better, and is what `dotnet-web-backend` assumes.
