...
|---|---|---|
| Solution designers | the house domain skills (preloaded), the context7 MCP for a version-gated API or feature, `docs/architecture/ARCHITECTURE.md` plus its `docs/architecture/references/` detail (the durable map architecture-analyzer maintains) and serena for the repo's existing architecture, a memory recall of the prior frozen contract | a re-derived architecture, a wrong version assumption, a re-frozen contract |
