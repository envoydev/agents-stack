...
3. **Match the packages to the framework.** Update Microsoft and third-party packages to the line that targets the new framework, build, and work through the breaks. The official breaking-changes list for the release is the map; read it rather than guessing at each error. For a large or legacy solution the .NET Upgrade Assistant (`dotnet tool install -g upgrade-assistant`) automates the TFM and package sweep and surfaces the analyzers - use it to drive the mechanical part, then still read the breaking-changes list for the behavioral ones it cannot catch.
...
4. **Centralize versions.** With central package management the version lives once in `Directory.Packages.props`, not scattered across `.csproj` files - that mechanism is `package-management`.
