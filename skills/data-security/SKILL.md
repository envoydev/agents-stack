---
name: data-security
description: "Personal SQL / data-layer security-hardening reference, organized by the persistence threat surface: SQL injection closed at every sink (parameterized only, no interpolated FromSqlRaw / ExecuteSqlRaw, sp_executesql for dynamic proc SQL), least-privilege database accounts (never sa / db_owner, split migration and runtime logins), row-level security and tenant isolation enforced at the database or a global query filter (data-layer IDOR), secrets kept out of connection strings (integrated auth / managed identity / Key Vault, never a plaintext password in source), encryption at rest and in transit (TDE, column-level / Always Encrypted, Encrypt=true), sensitive-data exposure and dynamic data masking, and audit-logging that never records the secret. Load when hardening or reviewing a SQL / data-persistence feature, or when the security-auditor sweeps the data stack. Points at dotnet-security for the app-layer EF and injection surface, dotnet-cryptography for crypto primitives, and dotnet-migrate for safe migration mechanics. Do NOT load for non-security work."
---
