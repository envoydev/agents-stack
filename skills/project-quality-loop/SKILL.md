...
- The main session is the only orchestrator - never instruct a subagent to dispatch another (the shared flat-fan-out policy `subagent-flow` owns); the auditors and implementers this loop dispatches (domain verifiers, implementers, resolvers, analyzers) carry no Agent tool. A stage needing a verdict and a fix is two dispatches from here, not one nested one.
