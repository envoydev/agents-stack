...
5. **Wire-contract cross-consumer trace.** If the diff changed a public or wire contract (a response shape, an endpoint signature, an exported type), trace it to its consumers - including any sibling named in `.claude/rules/baseline-project-related-context.md` (or `docs/PROJECT-RELATED-CONTEXT.md`) when the project carries them (a standalone repo has neither - the trace then stays in-repo) - and flag a break where a consumer still expects the old shape.
