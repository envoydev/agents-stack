...
- Check out the PR head ref before any repro (`git fetch origin <head>` then `git checkout <head>`), stashing a dirty tree and recording the stash + the original ref in your report - read-only means file contents and config; working-tree prep for a faithful repro is sanctioned, because repro'ing against the wrong checkout diagnoses the wrong code.
...
The CI signature catalogue lives in the preloaded `project-ci-failure-signatures` skill; it is the single source for both this seat and single-chat triage; do not restate it here. Match each failing job's digest to its signature and make the code-vs-environment call there. This seat adds what single-chat cannot: the per-job evidence-gatherer fan-out and the one bounded local-repro attempt.
...
Open with a diagnosis status - DIAGNOSED, CI_PASSING, LIKELY_FLAKE, NEEDS_MORE_EVIDENCE, or INCONCLUSIVE - the base set `project-solve-cross-task`'s `references/issue-investigation.md` defines, with CI_PASSING replacing its NOT_REPRODUCED for the green-first guard - then per failing job, its category, the evidence quoted from the logs, the local repro result (reproduced, ran but did not reproduce, or no local equivalent), and the route the issue-flow defines - a resolver when it reproduced as code, the session when the diagnosis is an environment/pin/workflow delta, the domain verifier to re-gate; agent names plain, not backticked. Account for the gathering: state how many evidence-gatherers you dispatched and their summed `subagent_tokens` / `tool_uses` (from each gather-task's result), so the diagnosis's true cost - your reasoning plus the delegated gathering - is visible, not buried in the nested subtree.
