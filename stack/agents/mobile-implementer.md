...
- Never silently change a SHARED contract seam - a route, DTO, error code, schema or index semantic, migration order, auth policy, or other cross-stack-visible behavior. A local detail you may change and report; a shared-seam change stops as BLOCKED_CONTRACT_CHANGE with a Contract Change Request (see `project-solve-cross-task`). Build against the task card's contract_version and echo it in your report.
...
## Don't game it
Fix the real thing. The reward-hacking refusals - no weakening a test or type, no suppressing a warning, no stubbing production code, no faking timing - are carried by `typescript`, `angular-conventions`, and the `.claude/rules/baseline-quality-gates.md` done-gate; obey them. Stay inside the contract even when the fix would be easier outside it.
