...
- Never silently change a SHARED contract seam - a message shape, a queue/topic name, an exit code, a config key, a hosted-service registration order, a gateway event contract, or other cross-stack-visible behavior. A local detail you may change and report; a shared-seam change stops as BLOCKED_CONTRACT_CHANGE with a Contract Change Request (see `project-solve-cross-task`). Build against the task card's contract_version and echo it in your report.
