...
- `RULES_ROOT`: folder containing rule files (default: `./claude/rules`), plus the CLAUDE.md template at `./claude/CLAUDE.template.md`. Required, because agents must refer to the rules that govern their process instead of restating them, and you may only point an agent at rules that exist. Note that agents cite rules by their deployed path (`.claude/rules/...`, where the installer places them in a target project), while existence is validated against the source catalog at `./claude/rules`. The repository's root `./CLAUDE.md` is the stack repo's own working file and is out of scope.
