#!/usr/bin/env bash
#
# CURATED PROFILE - web-angular (Angular web / SPA development).
#
# This is a curated, standalone copy DERIVED FROM claude/claude-stack.sh (the SOURCE OF TRUTH).
# It is the full installer with the manifest entries irrelevant to Angular web work COMMENTED OUT
# (prepend '#', not deleted - so the trim stays visible and reversible) and the convention-hook
# variant args trimmed to this domain. All install logic, functions, and prereq checks are
# byte-identical to the source.
#
# Regenerate this profile whenever the main inventory (claude/claude-stack.sh) changes.
# The parity lint does NOT cover this file.
#
# Usage - run this file directly inside the target project:
#   bash web-angular.sh install   # install for Claude Code
#   bash web-angular.sh update    # update Claude Code (skills + plugins + mcp + hooks)
set -euo pipefail
...
HOOKS=(
  ...
  "require-convention-skill.js::Edit|Write|mcp__serena__replace_symbol_body|mcp__serena__insert_after_symbol|mcp__serena__insert_before_symbol::ng ts"
