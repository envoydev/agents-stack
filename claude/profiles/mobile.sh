#!/usr/bin/env bash
#
# CURATED PROFILE - mobile (Angular mobile: Ionic / Capacitor development).
# Derived from claude/claude-stack.sh (the SOURCE OF TRUTH); standalone-runnable. The manifest
# below is the full inventory trimmed to this domain: irrelevant entries are COMMENTED OUT (the
# trim stays visible and reversible) and the convention-hook args are narrowed to this domain.
# Regenerate this file when the main inventory (claude/claude-stack.sh) changes; the parity lint
# does NOT cover it. Everything else is byte-identical to the source.
set -euo pipefail
