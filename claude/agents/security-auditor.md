...
tools: Read, Skill, Bash, Grep, Glob, mcp__serena__find_symbol, mcp__serena__find_referencing_symbols, mcp__serena__get_symbols_overview, mcp__serena__write_memory, mcp__serena__read_memory, mcp__serena__list_memories, mcp__context7__*, WebSearch, WebFetch
model: opus
effort: xhigh
color: red
---
...
- **IDOR behind an honest UI** - the screen only shows the caller's own records, but the API still accepts any `id`; the missing control is a server-side ownership check, not a client-side filter. Isolate at the handler, not the view.
...
- **Over-posting / mass assignment** - a bind straight to the EF entity lets a request set `IsAdmin` / `Role` / `OwnerId`; the fix is a request DTO, not `[Bind]` gymnastics.
...
- **Desktop trust boundary (WPF)** - a `BinaryFormatter` / `SoapFormatter` deserializing data that crossed a trust boundary (a documented RCE sink - move to `System.Text.Json` with a known type); a secret in plaintext `app.config` or the registry instead of DPAPI-protected storage; a `WebView2` pointed at attacker-influenced content with `CoreWebView2` host objects exposed; an unsigned or HTTP ClickOnce update channel an attacker can substitute.
