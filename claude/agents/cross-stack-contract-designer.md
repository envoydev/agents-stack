...
- Read `docs/architecture/ARCHITECTURE.md` at START if it exists - the durable project-architecture map architecture-analyzer maintains (framework and packages, architecture logic, project structure, patterns, boundaries). Honor it so the contract stays consistent with the structure already in place, and flag in your report where this work forces the doc to change so architecture-analyzer can fold it in; it never replaces your own serena navigation of the specific code.
