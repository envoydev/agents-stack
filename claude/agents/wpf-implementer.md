...
- Memory handoff - a durable cross-run, cross-project recall layer on top of the dispatch-in / report-out path, not a replacement for it. At START, search the memory MCP for prior memories tagged with this feature and its contract_version and read any that touch your task's seams. At HAND-OFF, store one compact memory tagged with the feature, contract_version, and this seat, capturing notable cross-cutting findings, any contract deviations you reported, and decisions you made under the contract - reusable and compact, never a dump of the diff.
